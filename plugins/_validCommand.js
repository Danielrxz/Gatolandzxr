...
export async function before(m) {
  ...
  if (validCommand(command, global.plugins)) {
    ...
    if (chat.isBanned) {
      const aviso = `🚫 *${botname}* está desactivado en este grupo, ⚔️\n\n🛡️ Un *admin-sama* puede activarlo con:\n➤ *${usedPrefix}bot on*`
      await m.reply(aviso)
      return
    }
    ...
    respuesta += `⚔️ El comando *${comando}* no existe tarado.\n`
    respuesta += `☁️ Usa *${usedPrefix}help* para ver la lista completa de comandos🦥.\n`

    if (sugerencias.length > 0) {
      respuesta += `\n☁️ Quizás quisiste decir:\n`
