...
const handler = async (m, { conn, usedPrefix, command, args, isOwner, isAdmin, isROwner }) => {
  ...
  if (args[0] === 'on' || args[0] === 'enable') {
    ...
    const estado = isEnable ? '🚀 𝑨ctivado' : '👀 𝑫𝒆𝒔𝒂𝒄𝒕𝒊𝒗𝒂𝒅𝒂'
    return conn.reply(m.chat, `⚔️・*Opciones para ${command}*・🌸\n\n` +
    `👻 Usa:\n` +
  ...
  conn.reply(m.chat, `🌸 La opción *${type}* ha sido *${isEnable ? 'encendida ⚔️' : 'apagada ☁️'}* ${isAll ? 'para el bot~' : 'en este grupo bonito~ 😼'}`, m)
}
